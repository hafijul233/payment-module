...
class CurlRequest extends Request implements RequestInterface
{
    ...
    protected function setCURLOptions(array $curlOptions = [], array $config = [])
    {
        ...
        if (! empty($config['cert'])) {
            ...
            if (! is_file($cert)) {
                throw RequestException::forSSLCertNotFound($cert);
            }
        ...
        if (isset($config['verify'])) {
            if (is_string($config['verify'])) {
                ...
                if (! is_file($file)) {
                    throw RequestException::forInvalidSSLKey($config['ssl_key']);
                }
    ...
    protected function sendRequest(array $curlOptions = []): string
    {
        ...
        if ($output === false) {
            throw RequestException::forCurlError((string) curl_errno($ch), curl_error($ch));
        }
